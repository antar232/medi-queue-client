        ...
        <nav className="sticky top-0 z-50 w-full border-b border-slate-200 bg-white shadow-sm dark:border-emerald-950 dark:bg-[#022c22] transition-colors duration-300">
                ...
                <div className="flex h-16 items-center justify-between">
                    ...
                    <div className="flex-shrink-0">
                        <Link href="/" className="flex items-center gap-2 text-xl font-bold tracking-tight text-slate-900 dark:text-white focus:outline-none">
                            ...
                            <span className="text-emerald-700 dark:text-emerald-400">Medi</span>Queue
                        </Link>
                    </div>
                    ...
                    <div className="hidden md:flex items-center space-x-4">
                        ...
                        >
                            {isDarkMode ? (
                                <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707-.707M6.343 6.343l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.226.226a5 5 0 01-7.072 0z"/></svg>
                            ) : (
                            ...
                            <div className="relative flex items-center gap-3 ml-2">
                                ...
                                {isProfileOpen && (
                                    <div className="absolute right-0 top-10 z-50 mt-2 w-56 origin-top-right rounded-lg bg-white py-1 shadow-xl border border-slate-100 dark:bg-[#022c22] dark:border-emerald-800">
                                        ...
                                        <Link href="/my-bookings" className="flex items-center gap-2 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-emerald-900/40" onClick={() => setIsProfileOpen(false)}>💳 My bookings</Link>
                                        <Link href="/my-tutors" className="flex items-center gap-2 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-emerald-900/40" onClick={() => setIsProfileOpen(false)}>📖 My tutors</Link>
                                        <button onClick={handleLogout} className="flex w-full items-center gap-2 px-4 py-2.5 text-sm text-left font-medium text-rose-600 border-t border-slate-100 dark:border-emerald-900 dark:text-rose-400 hover:bg-slate-50 dark:hover:bg-emerald-900/40">
                                            🚪 Log out
                                        </button>
                                    </div>
                                )}
                    ...
                    <div className="flex md:hidden items-center space-x-2">
                        ...
                        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="inline-flex items-center justify-center rounded-lg border border-slate-200 p-2 text-slate-500 hover:bg-slate-50 dark:border-emerald-800 dark:text-emerald-400 dark:hover:bg-emerald-900">
            ...
            {isMobileMenuOpen && (
                <div className="md:hidden border-t border-slate-200 bg-white px-4 py-4 space-y-2 dark:border-emerald-900 dark:bg-[#022c22] flex flex-col items-stretch">
                    ...
                    <CustomNavLink to="/tutors" onClick={() => setIsMobileMenuOpen(false)}>📚 Tutors</CustomNavLink>
                    
                    {user ? (
                        <>
                            <CustomNavLink to="/add-tutor" onClick={() => setIsMobileMenuOpen(false)}>➕ Add tutor</CustomNavLink>
                            <CustomNavLink to="/my-tutors" onClick={() => setIsMobileMenuOpen(false)}>📖 My tutors</CustomNavLink>
                            <CustomNavLink to="/my-bookings" onClick={() => setIsMobileMenuOpen(false)}>💳 My bookings</CustomNavLink>
