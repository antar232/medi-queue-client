...
const SignUpPage = () => {
  ...
  const onSubmit = async (e) => {
    ...
    if (
      ...
      setServerError(
        "Please satisfy all password criteria before registering.",
      );
      return;
    }
        ...
        <div className="w-full md:w-7/12 p-8 sm:p-12 flex flex-col justify-center">
          ...
          <Form onSubmit={onSubmit} className="flex flex-col gap-4 w-full">
            ...
            <div className="p-3 bg-emerald-50/50 dark:bg-emerald-950/20 rounded-lg border border-emerald-100/70 dark:border-emerald-900/40 space-y-1.5">
              <div
                className={`flex items-center gap-2 text-xs font-medium transition-colors ${validationChecks.length ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500"}`}
              >
                <span>{validationChecks.length ? "✓" : "✕"}</span> At least 6
                characters
              </div>
              <div
                className={`flex items-center gap-2 text-xs font-medium transition-colors ${validationChecks.lowercase ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500"}`}
              >
                <span>{validationChecks.lowercase ? "✓" : "✕"}</span> Contains a
                lowercase letter
              </div>
              <div
                className={`flex items-center gap-2 text-xs font-medium transition-colors ${validationChecks.uppercase ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500"}`}
              >
                <span>{validationChecks.uppercase ? "✓" : "✕"}</span> Contains
                an uppercase letter
              </div>
            </div>
