    ...
    <div className="bg-[#fcfbf7] p-6 rounded-2xl border border-[#f5f2e9] space-y-4">
      ...
      {isSlotsZero ? (
        <div className="p-4 bg-red-50 border border-red-200 text-red-600 rounded-xl text-xs font-bold">
          ❌ No available slots left. This session is fully booked. You can’t
          join at the moment.
        </div>
      ) : isDateEarly ? (
        <div className="p-4 bg-yellow-50 border border-yellow-200 text-yellow-700 rounded-xl text-xs font-bold">
          🕒 Booking is not available yet for this tutor. (Opens after session
          start date)
        </div>
      ) : null}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <Input
              type="text"
              value={
                tutorId ? `#${tutorId.slice(-6).toUpperCase()}` : "#TUT001"
              }
              readOnly
              variant="faded"
